    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
            sub.setName("diverso")
                ...
                .addStringOption(opt => 
                    opt.setName("link")
                       .setDescription("Link da imagem ou ficha do item")
                       .setRequired(true)
                )
        ),
        ...
        try {
            ...
            if (subcomando === "dinheiro") {
                ...
                collector.on('collect', async iSelect => {
                    ...
                    try {
                        const modalSubmit = await iSelect.awaitModalSubmit({ filter: m => m.customId === modalId && m.user.id === interaction.user.id, time: 60000 });
                        ...
                        if (charAtualizado.saldo < valor) {
                            return modalSubmit.followUp({ content: "💸 Você não tem saldo suficiente para essa transferência.", flags: MessageFlags.Ephemeral });
                        }

                        await prisma.$transaction([
                            prisma.personagens.update({ where: { id: charAtivo.id }, data: { saldo: { decrement: valor } } }),
                            prisma.transacao.create({ data: { personagem_id: charAtivo.id, descricao: `Enviou para Alt (${destinoAlt.nome})`, valor: valor, tipo: "GASTO" } }),
                            prisma.personagens.update({ where: { id: destinoId }, data: { saldo: { increment: valor } } }),
                            prisma.transacao.create({ data: { personagem_id: destinoId, descricao: `Recebeu do Alt (${charAtivo.nome})`, valor: valor, tipo: "GANHO" } })
                        ]);
            ...
            if (subcomando === "ingrediente") {
                ...
                listaItens.slice(0, 25).forEach(nomeItem => {
                    menuItens.addOptions(new StringSelectMenuOptionBuilder().setLabel(nomeItem).setDescription(`Você tem: ${estoque[nomeItem]}`).setValue(nomeItem));
                });
                ...
                meusAlts.forEach(alt => menuAlts.addOptions(new StringSelectMenuOptionBuilder().setLabel(alt.nome).setValue(String(alt.id))));
                ...
                collector.on('collect', async iComp => {
                    ...
                    if (iComp.isButton() && iComp.customId.startsWith("btn_confirmar_alt_")) {
                        ...
                        try {
                            const modalSubmit = await iComp.awaitModalSubmit({ filter: m => m.customId === modalId && m.user.id === interaction.user.id, time: 60000 });
                            await modalSubmit.deferUpdate();

                            const qtdEnviar = parseInt(modalSubmit.fields.getTextInputValue('inp_qtd'));

                            if (isNaN(qtdEnviar) || qtdEnviar <= 0 || qtdEnviar > maxDisp) {
                                return modalSubmit.followUp({ content: `🚫 Quantidade inválida. Você só tem ${maxDisp} disponíveis.`, flags: MessageFlags.Ephemeral });
                            }

                            const charOrigemAtualizado = await prisma.personagens.findUnique({ where: { id: charAtivo.id } });
                            const charDestinoAtualizado = await prisma.personagens.findUnique({ where: { id: destinoEscolhidoId } });
                            ...
                            if (!estOrigem[itemEscolhido] || estOrigem[itemEscolhido] < qtdEnviar) {
                                return modalSubmit.followUp({ content: "❌ Ocorreu um erro: o item não está mais no seu estoque.", flags: MessageFlags.Ephemeral });
                            }
                            ...
                            await prisma.$transaction([
                                prisma.personagens.update({ where: { id: charAtivo.id }, data: { estoque_ingredientes: estOrigem } }),
                                prisma.personagens.update({ where: { id: destinoEscolhidoId }, data: { estoque_ingredientes: estDestino } })
                            ]);
