                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: "\u200B", value: "**Atributos**" },
                        {
                            name: "Físicos",
                            value:
                                `**FOR:** ${p.forca >= 0 ? "+" : ""}${p.forca} (CD ${calcCD(p.forca)})\n` +
                        ...
                        },
                        {
                            name: "Mentais",
                            value:
                                `**INT:** ${p.inteligencia >= 0 ? "+" : ""}${p.inteligencia} (CD ${calcCD(p.inteligencia)})\n` +
            ...
            collector.on("collect", async iBtn => {
                ...
                if (iBtn.customId === "edit_pericias") {
                    ...
                    PERICIAS_LISTA_1.forEach(p =>
                        menu1.addOptions(new StringSelectMenuOptionBuilder().setLabel(p).setValue(p))
                    );
                    PERICIAS_LISTA_2.forEach(p =>
                        menu2.addOptions(new StringSelectMenuOptionBuilder().setLabel(p).setValue(p))
                    );
                ...
                if (iBtn.customId === "edit_classes") {
                    ...
                    LISTA_CLASSES_1.forEach(cls =>
                        menu1.addOptions(new StringSelectMenuOptionBuilder().setLabel(cls).setValue(cls))
                    );
                    LISTA_CLASSES_2.forEach(cls =>
                        menu2.addOptions(new StringSelectMenuOptionBuilder().setLabel(cls).setValue(cls))
                    );
                        ...
                        try {
                            ...
                            if (nivelInput > 0) {
                                const somaAtual = charAtual.classes.reduce(
                                    (acc, c) => acc + (c.nome_classe === classeSelecionada ? 0 : c.nivel),
                                    0
                                );
                ...
                if (iBtn.customId === "btn_descanso") {
                    ...
                    const botoesDescanso = new ActionRowBuilder().addComponents(
                        new ButtonBuilder()
                            .setCustomId("desc_ruim")
                            .setLabel("Ruim (Nív/2)")
                            .setStyle(ButtonStyle.Secondary),
                        new ButtonBuilder()
                            .setCustomId("desc_normal")
                            .setLabel("Normal (Nív)")
                            .setStyle(ButtonStyle.Primary),
                        new ButtonBuilder()
                            .setCustomId("desc_conf")
                            .setLabel("Confortável (2x)")
                            .setStyle(ButtonStyle.Success),
                        new ButtonBuilder()
                            .setCustomId("desc_lux")
                            .setLabel("Luxuoso (3x)")
                            .setStyle(ButtonStyle.Success)
                    );
                ...
                if (iBtn.customId === "edit_fisico") {
                        ...
                        .addComponents(
                            ...
                            new ActionRowBuilder().addComponents(
                                new TextInputBuilder()
                                    ...
                                    .setStyle(TextInputStyle.Short)
                                    .setValue(String(char.constituicao))
                            )
                        );
                    ...
                    try {
                        ...
                        char = await prisma.personagens.findFirst({
                            where: { id: char.id },
                            include: { classes: true }
                        });
                        await interaction.editReply({ embeds: [montarEmbedFicha(char)] });
                        ...
                        .addComponents(
                            new ActionRowBuilder().addComponents(
                                new TextInputBuilder()
                                    ...
                                    .setStyle(TextInputStyle.Short)
                                    .setValue(String(char.inteligencia))
                            ),
                            new ActionRowBuilder().addComponents(
                                new TextInputBuilder()
                                    .setCustomId("inp_sab")
                                    .setLabel("Sabedoria")
                                    .setStyle(TextInputStyle.Short)
                                    .setValue(String(char.sabedoria))
                            ),
                            new ActionRowBuilder().addComponents(
                                new TextInputBuilder()
                                    .setCustomId("inp_car")
                ...
                if (iBtn.customId === "edit_deslocamento") {
                    ...
                    try {
                        ...
                        const textoDeslocamento = modalSubmit.fields
                            .getTextInputValue("inp_deslocamento")
                            .replace(",", ".");
