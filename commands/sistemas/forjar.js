...
module.exports = {
    ...
    async execute({
        ...
        try {
            ...
            if (!char)
                return message.reply("Sem personagem ativo.").catch(()=>{});
            ...
            collector.on("collect", async i => {
                ...
                const modal = new ModalBuilder()
                    .setCustomId(`modal_forja_${tipoSelecionado}`)
                    .setTitle(`Forjar: ${tipoSelecionado}`);
                ...
                try {
                    ...
                    const submit = await i.awaitModalSubmit({
                        filter: inter =>
                            inter.customId === `modal_forja_${tipoSelecionado}` &&
                    ...
                    const qtd = parseInt(
                        submit.fields.getTextInputValue("inp_qtd")
                    );

                    const custoOuro = parseFloat(
                        submit.fields.getTextInputValue("inp_ouro")
                    );
                    ...
                    const custoPontosTotal =
                        parseFloat((custoPontosUnit * qtd).toFixed(2));
                    ...
                    if (charAtual.saldo < custoOuro)
                        return submit.reply({
                            content:
                                `🚫 Kwanzas insuficientes! Você tem ` +
                                `${formatarMoeda(charAtual.saldo)}.`,
                            ephemeral: true
                        });
                    ...
                    if (charAtual.pontos_forja_atual < custoPontosTotal)
                        return submit.reply({
                            content:
                                ...
                                `Custa ${custoPontosTotal}, você tem ` +
                                `${charAtual.pontos_forja_atual.toFixed(1)}.`,
                            ephemeral: true
                        });
            ...
            message.reply("Ocorreu um erro ao abrir a forja.").catch(()=>{});
