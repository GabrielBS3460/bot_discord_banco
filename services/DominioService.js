...
class DominioService {
    ...
    async buscarPainel(personagemId) {
        ...
        if (dominio.mes_ultimo_turno !== mesAtual) {
            ...
            let acoesNovo = dominio.corte === "Rica" ? 3 : 2;
            ...
            if (dominio.nivel >= 3) {
                ...
                if (rolagem <= 2) {
                    ...
                    logEvento =
                        "🐺 **MONSTRO!** Uma criatura está aterrorizando as fazendas. Você precisa gastar uma ação para caçá-la nesta sessão!";
                    acoesNovo--;
                } else if (rolagem <= 15) {
                    ...
                    if (d6 === 1) {
                        logEvento =
                            "🌪️ **DESASTRE NATURAL!** Um terremoto/tornado atingiu suas terras. O Domínio perdeu 1 Nível!";
                        nivelNovo = Math.max(1, nivelNovo - 1);
                    } else if (d6 <= 3) {
                    ...
                    logEvento =
                        "✨ **FENÔMENO MÁGICO!** Algo bizarro aconteceu. Durante esse Mês, o regente ganha uma ação adicional!";
                    acoesNovo++;
                } else if (rolagem <= 26) {
                    logEvento =
                        "📜 **QUESTÃO DIPLOMÁTICA!** Um emissário de outro reino exige favores ou tributos. Voce gastou 1d3 LO com suborno!";
                    tesouroNovo = Math.max(0, tesouroNovo - (Math.floor(Math.random() * 3) + 1));
                } else if (rolagem <= 30) {
                    logEvento =
                        "🔥 **LEVANTE!** A população se enfureceu com boatos. Sua popularidade caiu 2 categorias!";
                    ...
                    logEvento =
                        "⚖️ **QUESTÃO DE JUSTIÇA!** Uma disputa legal exige seu julgamento como regente. Você gastou uma ação para resolver.";
                    acoesNovo--;
                } else if (rolagem <= 52) {
                    logEvento =
                        "🥷 **BANDIDOS!** Salteadores estão roubando o povo. Você gastou uma ação para caça-los.";
                    acoesNovo--;
                } else if (rolagem <= 56) {
                    ...
                    logEvento =
                        "📉 **QUESTÃO COMERCIAL!** Uma crise de abastecimento estourou. Exige uma ação diplomática sua.";
                } else if (rolagem <= 70) {
                    ...
                    if (d6 <= 2) {
                        ...
                        logEvento =
                            "💡 **REGALIA: BOAS IDEIAS!** O povo ajudou o regente. Bônus em testes do domínio este mês.";
                    } else {
                        logEvento =
                            "🎉 **REGALIA: ARCO-ÍRIS!** Um clima de felicidade geral. A popularidade subiu 1 categoria!";
                        popNova = this.mudarPopularidade(dominio, 1);
                    }
                }
            } else {
                logEvento =
                    "🏕️ Seu domínio ainda é muito pacato e isolado para atrair a atenção do mundo (Eventos ocorrem a partir do Nível 3).";
            }

            acoesNovo = Math.max(0, acoesNovo);
    ...
    async construir(dominioId, charId, nomeConstrucao) {
        ...
        if (dominio.construcoes.length >= maxConstrucoes)
            throw new Error(
                `Seu domínio (Nível ${dominio.nivel}) já atingiu o limite de ${maxConstrucoes} construções.`
            );
        ...
        if (!pChar.includes(obra.tipo))
            throw new Error(`Você precisa ser treinado em **${obra.tipo}** para liderar esta obra.`);
        ...
        if (obra.reqTerreno) {
            ...
            if (!terrenosValidos.includes(dominio.terreno)) {
                ...
                if (!temPovoado) {
                    throw new Error(
                        `Esta obra exige o terreno: ${terrenosValidos.join(" ou ")} (Ou possuir a construção "Povoado Afastado").`
                    );
                }
            }
        }
    ...
    async recrutar(dominioId, nomeTropa, quantidade) {
        ...
        if (quantidade > dominio.nivel)
            throw new Error(
                `Seu Quartel-General (Nível ${dominio.nivel}) só consegue treinar ${dominio.nivel} unidades por ação!`
            );
        ...
        if (dominio.tesouro_lo < custoTotal)
            throw new Error(`O tesouro precisa de ${custoTotal} LO para bancar ${quantidade}x ${nomeTropa}.`);

        if (tropaData.req) {
            const possuiReq = dominio.construcoes.some(c => c.nome.toLowerCase() === tropaData.req.toLowerCase());
            if (!possuiReq)
                throw new Error(`Para recrutar **${nomeTropa}**, você precisa ter a construção: **${tropaData.req}**.`);
        }
