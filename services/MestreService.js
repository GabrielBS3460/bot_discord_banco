...
class MestreService {
    ...
    async gerarRelatorioMesa(nomeMissao) {
        const avaliacoes = await AvaliacaoRepository.buscarPorNomeMissao(nomeMissao);
        ...
        const medias = {
            ...
            geral: somas.geral / qtd,
            qtd,
            mestreId: avaliacoes[0].mestre_id,
            tipoContrato: avaliacoes[0].tipo_contrato,
            feedbacks: avaliacoes.map(a => a.feedback).filter(Boolean)
        };
